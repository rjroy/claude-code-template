...
SAID is fundamentally about **progressive certainty through iterative deepening**. Unlike waterfall (sequential phases) or agile (time-boxed iterations), the spiral method acknowledges that software development is a journey from uncertainty to clarity, where each level of detail reveals new questions and opportunities.
...
3. **Context Over Process**: Maintain rich context that survives team changes and timeline pressure
...
## Adapting SAID Under Pressure

Real projects face real pressures. SAID is designed to **degrade gracefully** rather than abandon structure entirely.

### The Pressure Adaptation Principle

Under pressure, you trade investigation depth for delivery speed, but **never abandon context**. A rushed project with preserved context can be improved. A rushed project with lost context must be rewritten.

### Pressure Response Patterns

**When You Have Weeks**: Full spiral method with thorough investigation at each level

**When You Have Days**: 
- Combine Levels 0 and 1 into rapid problem/solution validation
- Jump to Level 3 with documented assumptions
- Preserve critical decisions even if analysis is thin

**When You Have Hours**:
- Emergency mode: build the smallest thing that could possibly work
- Document every shortcut taken as technical debt
- Create recovery plan before pressure memory fades

### The Recovery Protocol

After a pressure-driven delivery:

1. **Immediate** (same day): Document what shortcuts were taken
2. **Short-term** (within a week): Create TODO items for deferred work
3. **Medium-term** (within a month): Address critical technical debt
4. **Long-term** (quarterly): Refactor based on learned reality

### Communicating Under Pressure

Pressure often comes from misaligned expectations. SAID helps by making trade-offs explicit:

- "We can deliver X by Friday if we defer Y and accept risk Z"
- "Here are three options with different speed/quality trade-offs"
- "This is what 'done' looks like at each timeline"

Stakeholders make better decisions when they understand consequences.
...
TODOs in SAID aren't just task tracking - they're **discovered work that emerges from the spiral process**.

### When TODOs Emerge

As you spiral through levels, you discover:
- **Level 0-1**: Research questions, spike investigations
- **Level 2-3**: Component boundaries, interface definitions
- **Level 4**: Implementation tasks, edge cases

Each discovery might spawn TODOs. The key is **capturing them without losing focus** on your current level.

### The TODO Spectrum

Not all TODOs need formal workflow:

**Simple TODOs** (just do them):
- Fix typo in documentation
- Add missing parameter validation
- Update deprecated dependency

**Complex TODOs** (need structure):
- Investigate authentication approaches
- Design caching strategy
- Refactor module with unclear boundaries

**Research TODOs** (need investigation):
- Evaluate third-party services
- Prototype performance optimization
- Analyze security implications

### TODO Workflow as Discovery Tool

The formal TODO workflow helps when you need to:
- Break down work that's too large to hold in your head
- Coordinate with team members on shared tasks
- Ensure complex work has clear success criteria
- Track decisions made during implementation

### Integration with Spiral Levels

TODOs can trigger spiral navigation:
- A simple TODO might reveal complexity requiring Level 1 analysis
- A research TODO might surface risks requiring Level 0 reconsideration
- Implementation TODOs might expose design flaws requiring Level 2 revision

This is the spiral method working correctly - **discovery drives navigation**.
